...
custom_round_monitor()
{
	...
	level.window1 = (-4400, -8000, -62.875);
	level.window2 = (-5030, -8000, -62.875);
	level.window3 = (-5569, -8000, 0.125);
	level.window4 = (-6430, -8000, 0.125);
	level.customround = 4;
	for(;;)
	{
		...
		if( level.round_number == level.customround ) 
		{
			...
			while(level.customround == level.round_number)
			{
				foreach( zombie in getAiArray(level.zombie_team) )
				{
					...
					if(distance( zombie.origin, level.window1 ) < 160)
					{
						zombie forceteleport(( -3980, -6958, -66.44 ));
						zombie thread find_flesh();
					}
					if(distance( zombie.origin, level.window2 ) < 160)
					{
						zombie forceteleport(( -3980, -6958, -66.44 ));
						zombie thread find_flesh();
					}
					if(distance( zombie.origin, level.window3 ) < 160)
					{
						zombie forceteleport(( -3980, -6958, -66.44 ));
						zombie thread find_flesh();
					}
					if(distance( zombie.origin, level.window4 ) < 160)
					{
						zombie forceteleport(( -3980, -6958, -66.44 ));
						zombie thread find_flesh();
					}
			...
			level.customround = level.round_number + randomintrange( 6, 8 );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				...
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Double Tap Root Beer [Cost: " + level.doubletapcost + "]" );						
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Jugger-Nog [Cost: " +  level.juggprice + "]" );
		...
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
                    if(players.size > 1)
                    {
						player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Revive [Cost: " +  level.mquickrevivecost + "]" );
                    }
                    else
                    {
					    player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Revive [Cost: " +  level.squickrevivecost + "]" );
                    }
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Stamin-Up [Cost: " +  level.stupcost + "]" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 )
				{	
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Tombstone Soda [Cost: " +  level.tombcost + "]" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for PhD Flopper [Cost: " +  level.phdcost + "]" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Downer's Delight [Cost: " +  level.DDprice + "]" );
                    player thread machine_lower_hint( "Players bleedout time is increased by 10 seconds and current weapons used in laststand");
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Victorious Tortoise [Cost: " +  level.VTprice + "]" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Electric Cherry [Cost: " +  level.cherrycost + "]" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Widow's Wine [Cost: " +  level.wwcost + "]" );
                    player thread machine_lower_hint( "Damages zombies around the player when hit and grenades are upgraded" );
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Ethereal Razor [Cost: " +  level.ERprice + "]" );
                    player thread machine_lower_hint( "Players melee attacks do extra damage and restore a small amount of health.");
	...
	while( 1 )
	{
		foreach( player in level.players )
		{
			if(!player.machine_is_in_use)
			{
				if( distance( self.origin, player.origin ) <= 60 ) 
				{
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Mule Kick [Cost: " +  level.t3guncost + "]" );
                    player thread machine_lower_hint( "Enables additional primary weapon slot for player " );

					if( player usebuttonpressed() && !(player.has_mule) && ( player.score >= level.t3guncost ) && !(self.lock) && !player maps/mp/zombies/_zm_laststand::player_is_in_laststand())
					{
						player.machine_is_in_use = 1;
						self.lock = 1;
						player playsound( "zmb_cha_ching" );
						...
						player playsound ( "mus_perks_mulekick_sting" );
						player thread drawshader_and_shadermove("MULE", 1);
						wait 1;
						weapona = self getcurrentweapon();
						self switchToWeapon(weapona);
						wait 3;
...
removeperkshader()
{
    for(;;)
    {
		...
		if(self.tombstone_save)
		{
			self.last_death_perks = [];
			self.tomb = self.origin;
			if(self.has_dd)
			{
				self.last_death_perks[self.last_death_perks.size] = "Downers_Delight";
			}
			if(self.has_mule)
			{
				self.last_death_perks[self.last_death_perks.size] = "MULE";
			}
			if(self.has_phd)
			{
				self.last_death_perks[self.last_death_perks.size] = "PHD_FLOPPER";
			}
			if(self.has_tortoise)
			{
				self.last_death_perks[self.last_death_perks.size] = "Victorious_Tortoise";
			}
			if(self.has_cherry)
			{
				self.last_death_perks[self.last_death_perks.size] = "ELECTRIC_CHERRY";
			}
			if(self.has_wine)
			{
				self.last_death_perks[self.last_death_perks.size] = "WIDOWS_WINE";
			}
			if(self.has_razor)
			{
				self.last_death_perks[self.last_death_perks.size] = "Ethereal_Razor";
			}
		}
        self.num_perks = 0;
        self.perk_reminder = 0;
...
drawshader_and_shadermove(perk, custom)
{
    if(custom)
	{
...
tombstone_perkcheck_and_give()
{
	...
	if(self.has_dd)
	{
		self.has_dd = 0;
      	self.perk1front destroy();
		if(!isinarray(self.last_death_perks, "Downers_Delight"))
		{
			self.last_death_perks[self.last_death_perks.size] = "Downers_Delight";
		}
	}
	if(self.has_mule)
	{
		self.has_mule = 0;
       	self.perk2front destroy();
		if(!isinarray(self.last_death_perks, "MULE"))
		{
			self.last_death_perks[self.last_death_perks.size] = "MULE";
		}
	}
	if(self.has_phd)
	{
		self.has_phd = 0;
        self.perk3front destroy();
		if(!isinarray(self.last_death_perks, "PHD_FLOPPER"))
		{
			self.last_death_perks[self.last_death_perks.size] = "PHD_FLOPPER";
		}
	}
	if(self.has_tortoise)
	{
		self.has_tortoise = 0;
		self.perk4front destroy();
		if(!isinarray(self.last_death_perks, "Victorious_Tortoise"))
		{
			self.last_death_perks[self.last_death_perks.size] = "Victorious_Tortoise";
		}
	}
	if(self.has_cherry)
	{
		self.has_cherry = 0;
		self.perk5front destroy();
		if(!isinarray(self.last_death_perks, "ELECTRIC_CHERRY"))
		{
			self.last_death_perks[self.last_death_perks.size] = "ELECTRIC_CHERRY";
		}
	}
	if(self.has_wine)
	{
		self.has_wine = 0;
		self.perk6front destroy();
		if(!isinarray(self.last_death_perks, "WIDOWS_WINE"))
		{
			self.last_death_perks[self.last_death_perks.size] = "WIDOWS_WINE";
		}
	}
	if(self.has_razor)
	{
		self.has_razor = 0;
		self.perk7front destroy();
		if(!isinarray(self.last_death_perks, "Ethereal_Razor"))
		{
			self.last_death_perks[self.last_death_perks.size] = "Ethereal_Razor";
		}
	}
	...
	{
        self drawshader_and_shadermove(self.last_death_perks[i], 0);
    }
	self.last_death_perks = [];
}
    ...
    while( 1 )
    {
        foreach( player in level.players )
        {
            if(!player.machine_is_in_use)
            {
                if( distance( self.origin, player.origin ) <= 60 ) 
                {
					player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for Pack-A-Punch [Cost: " + level.papprice + "]" );
                    if(player usebuttonpressed() && player.score >= level.papprice && !player maps/mp/zombies/_zm_laststand::player_is_in_laststand())
					{
        				...
        				for(i = 0; i < level.custom_gunlist.size; i++)
						{
            				if(currgun == level.custom_gunlist[i])
							{
                                ...
                                player giveweapon(player maps/mp/zombies/_zm_weapons::get_upgrade_weapon( currgun, 0 ), 0, player custom_get_pack_a_punch_weapon_options(level.custom_gunlist_upgraded[i]));
...
custom_get_pack_a_punch_weapon_options( weapon )
{
	...
	if( base == "qcw05_upgraded_zm" || weapon == "qcw05_zm" || base == "fivesevendw_upgraded_zm" || weapon == "fivesevendw_zm" || base == "fiveseven_upgraded_zm" || weapon == "fiveseven_zm" || base == "m32_upgraded_zm" || weapon == "m32_zm" || base == "ray_gun_upgraded_zm" || weapon == "ray_gun_zm" || base == "raygun_mark2_upgraded_zm" || weapon == "raygun_mark2_zm" || base == "m1911_upgraded_zm" || weapon == "m1911_zm" || base == "knife_ballistic_upgraded_zm" || weapon == "knife_ballistic_zm")
	{
		camo_index = 39;
	}
	else
...
wallweaponmonitorbox( weapon, cost )
{
    ...
    while( 1 )
    {
        foreach( player in level.players )
        {
            ...
            {
				player thread SpawnHint( self.origin, 30, 30, "HINT_ACTIVATE", "Hold ^3&&1^7 for " + self.weapx + " [Cost: " + self.cost + "]" );
